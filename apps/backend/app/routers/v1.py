...
def create_memory(req: MemoryCreateRequest) -> MemoryResponse:
    ...
    memory = service.save_memory(user_id=req.user_id, content=req.content, source=req.source)
    return MemoryResponse(id=memory.id, user_id=memory.user_id, content=memory.content, source=memory.source)
